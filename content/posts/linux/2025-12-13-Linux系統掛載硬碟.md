...
```bash
# 用 parted 針對 /dev/sdb 進行操作，以下指令將進入 parted 指令介面
parted /dev/sdb
...
使用 mkfs.ext4 指令將處理好的分割區 /dev/sdb1 格式化為 ext4 檔案系統。
...
- 手動掛載(暫時掛載)：每次開機時需要用 mount 指令將 /sdb/sdb1 掛載到目錄。
...
```bash
# 若有手動掛載，則先取消掛載
# umount 也可指定裝置名稱 /sdb/sdb1，但建議指定掛載目錄比較不容易出錯
...
若成功掛載，則每次系統開機都會自動將 /sdb/sdb1 掛載到 /mnt/new_storage。
